...
(function() {
    "use strict";
    if (typeof ALPHA_COLOR_STORAGE == 'undefined')
        window.ALPHA_COLOR_STORAGE = {};
    ...
    window.alpha_color_storage_set_array = function(var_name, key, value) {
        if (!alpha_color_isset(ALPHA_COLOR_STORAGE[var_name]))
            ALPHA_COLOR_STORAGE[var_name] = {};
    ...
    window.dco === undefined && (window.dco = function(s, r) {
        console.log(alpha_color_debug_object(s, r));
    }
    );
    ...
    window.dao === undefined && (window.dao = function(s, r) {
        if (ALPHA_COLOR_STORAGE['user_logged_in'])
            alert(alpha_color_debug_object(s, r));
    }
    );
    window.ddl === undefined && (window.ddl = function(s, c) {
        alpha_color_debug_log(s, c);
    }
    );
    window.ddo === undefined && (window.ddo = function(s, r, c) {
        alpha_color_debug_log(alpha_color_debug_object(s, r), c);
    }
    );
}
)();
jQuery(document).ready(function() {
    ...
    function alpha_color_vc_tour() {
        jQuery('.vc_tta-change_on_hover').on('click', '.vc_tta-tab a', function() {
    ...
    function alpha_color_ready_actions() {
        ...
        jQuery('.alpha_color_tabs_ajax').on("tabsbeforeactivate", function(event, ui) {
            if (ui.newPanel.data('need-content'))
                alpha_color_tabs_ajax_content_loader(ui.newPanel, 1, ui.oldPanel);
        });
        ...
        jQuery('.sc_layouts_menu li[class*="image-"]').each(function() {
            ...
            if (icon)
                jQuery(this).find('>a').css('background-image', 'url(' + ALPHA_COLOR_STORAGE['theme_url'] + '/alpha_color/css/icons.png/' + icon + '.png');
        });
        ...
        jQuery('select:not(.esg-sorting-select):not([class*="alpha_color_attrib_"]):not(.country_to_state):not(.state_select)').each(function() {
            var s = jQuery(this);
            if (s.css('display') != 'none' && !s.next().hasClass('select2') && !s.hasClass('select2-hidden-accessible') && s.parents('.comment-form-rating').length != 1)
        ...
        jQuery('.nav-links-more a').on('click', function(e) {
            if (ALPHA_COLOR_STORAGE['load_more_link_busy'])
                return;
            ...
            if (panel.length == 0) {
                jQuery.get(location.href, {
                    ...
                    alpha_color_loadmore_add_items(jQuery('.content .posts_container').eq(0), jQuery(response).find('.content .posts_container > article,' + '.content .posts_container > div[class*="column-"],' + '.content .posts_container > .masonry_item'));
                });
            } else {
                jQuery.post(ALPHA_COLOR_STORAGE['ajax_url'], {
                    ...
                    try {
                        ...
                        rez = {
                            error: ALPHA_COLOR_STORAGE['strings']['ajax_error']
                        };
    ...
    function alpha_color_resize_actions(cont) {
        ...
        if (ALPHA_COLOR_STORAGE['menu_side_stretch'])
            alpha_color_stretch_sidemenu();
    ...
    function alpha_color_resize_video(cont) {
        ...
        cont.find('video').each(function() {
            ...
            if ($self.parents('.mejs-mediaelement').length === 0 || $self.hasClass('trx_addons_resize') || $self.hasClass('trx_addons_noresize') || $self.parents('div:hidden,section:hidden,article:hidden').length > 0) {
                return;
            }
            var video = $self.addClass('alpha_color_resize').eq(0);
        ...
        cont.find('.video_frame iframe,iframe').each(function() {
            var $self = jQuery(this);
            if ($self.hasClass('trx_addons_resize') || $self.hasClass('trx_addons_noresize') || $self.parent().is('rs-bgvideo') || $self.parents('rs-slide').length > 0 || $self.addClass('alpha_color_resize').parents('div:hidden,section:hidden,article:hidden').length > 0) {
                return;
            }
    ...
    function alpha_color_stretch_sidemenu() {
        ...
        if (toc_items.length < 5)
            return;
        var toc_items_height = jQuery(window).height() - alpha_color_fixed_rows_height(true, false) - jQuery('.menu_side_wrap .sc_layouts_logo').outerHeight() - toc_items.length;
    ...
    function alpha_color_vc_row_fullwidth_to_boxed(cont) {
        var page_wrap = jQuery('.page_wrap')
          , body = jQuery('body');
        if (body.hasClass('body_style_boxed') || body.hasClass('menu_style_side') || parseInt(page_wrap.css('paddingLeft'), 10) > 0) {
        ...
        if (sb.length > 0) {
            if (content.css('float') == 'none') {
                ...
                if (sb_height < content_height && scroll_offset + top_panel_fixed_height > content_top) {
                    ...
                    if (sb_top < 0)
                        sb_top = ALPHA_COLOR_STORAGE['sb_top_last'];
                    if (sb_big) {
                    ...
                    if (sb_init.position != 'undefined') {
                        ...
                        if (sb_init.position == 'fixed' || (!jQuery('body').hasClass('body_style_wide') && !jQuery('body').hasClass('body_style_boxed'))) {
                            ...
                            pos = pos.left + Math.max(0, parseInt(sb_parent.css('paddingLeft'), 10)) + Math.max(0, parseInt(sb_parent.css('marginLeft'), 10)) + (jQuery('body').hasClass('menu_style_right') ? Math.max(0, parseInt(jQuery('body').css('marginRight'), 10)) : 0);
                        }
    ...
    ALPHA_COLOR_STORAGE['mejs_attempts'] = 0;
    function alpha_color_init_media_elements(cont) {
        ...
        if (ALPHA_COLOR_STORAGE['use_mediaelements'] && cont.find(media_selector).length > 0) {
            if (window.mejs) {
                ...
                cont.find(media_selector).each(function() {
                    ...
                    if (!$self.hasClass('no-mejs') && !$self.hasClass('no-mediaelement') && !$self.hasClass('wp-block-cover__video-background') && $self.addClass('inited').parents('.mejs-mediaelement').length === 0 && $self.parents('.wp-block-video').length === 0 && $self.parents('.wp-block-media-text').length === 0 && $self.parents('.elementor-background-video-container').length === 0 && $self.parents('.elementor-widget-video').length === 0 && $self.parents('.with_video_autoplay').length === 0 && $self.parents('.sc_layouts_title').length === 0 && (ALPHA_COLOR_STORAGE['init_all_mediaelements'] || (!$self.hasClass('wp-audio-shortcode') && !$self.hasClass('wp-video-shortcode') && !$self.parent().hasClass('wp-playlist')))) {
            ...
            } else if (ALPHA_COLOR_STORAGE['mejs_attempts']++ < 5) {
                setTimeout(function() {
                    alpha_color_init_media_elements(cont);
                }, 400);
            }
    ...
    function alpha_color_comments_validate(form) {
        ...
        var comments_args = {
            error_message_text: ALPHA_COLOR_STORAGE['strings']['error_global'],
            error_message_show: true,
            error_message_time: 4000,
            error_message_class: 'alpha_color_messagebox alpha_color_messagebox_style_error',
            ...
            rules: [{
                ...
                min_length: {
                    value: 1,
                    message: ALPHA_COLOR_STORAGE['strings']['text_empty']
                },
                max_length: {
                    value: ALPHA_COLOR_STORAGE['comment_maxlength'],
                    message: ALPHA_COLOR_STORAGE['strings']['text_long']
                }
            }]
        };
        if (form.find('.comments_author input[aria-required="true"]').length > 0) {
            comments_args.rules.push({
                ...
                min_length: {
                    value: 1,
                    message: ALPHA_COLOR_STORAGE['strings']['name_empty']
                },
                max_length: {
                    value: 60,
                    message: ALPHA_COLOR_STORAGE['strings']['name_long']
                }
            });
        }
        if (form.find('.comments_email input[aria-required="true"]').length > 0) {
            comments_args.rules.push({
                ...
                min_length: {
                    value: 1,
                    message: ALPHA_COLOR_STORAGE['strings']['email_empty']
                },
                max_length: {
                    value: 60,
                    message: ALPHA_COLOR_STORAGE['strings']['email_long']
                },
                mask: {
                    value: ALPHA_COLOR_STORAGE['email_mask'],
                    message: ALPHA_COLOR_STORAGE['strings']['email_not_valid']
                }
            });
        }
...
jQuery(document).on('action.init_hidden_elements', function(e, cont) {
    ...
    if (ALPHA_COLOR_STORAGE['button_hover'] && ALPHA_COLOR_STORAGE['button_hover'] != 'default') {
        ...
        jQuery('.sidebar .trx_addons_tabs .trx_addons_tabs_titles li a:not([class*="sc_button_hover_style_"]),\ .alpha_color_tabs .alpha_color_tabs_titles li a:not([class*="sc_button_hover_style_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_style_alterbd');
        ...
        jQuery('.sc_button.color_style_link2:not([class*="sc_button_simple"]):not([class*="sc_button_hover_style_"]),\ .sc_price .trx_addons_columns_wrap > div:nth-child(3n+2) .sc_price_item .sc_price_item_link\ ').addClass('sc_button_hover_just_init sc_button_hover_style_link2');
        jQuery('.sc_button.color_style_link3:not([class*="sc_button_simple"]):not([class*="sc_button_hover_style_"]),\ .widget_tag_cloud a:not([class*="sc_button_hover_style_"]),\ .mc4wp-form .mc4wp-form-fields input[type="submit"],\ .wp-block-button:not(.is-style-outline) .wp-block-button__link,\ .sc_price .trx_addons_columns_wrap > div:nth-child(3n+1) .sc_price_item .sc_price_item_link,\ .widget_product_tag_cloud a:not([class*="sc_button_hover_style_"])\ ').addClass('sc_button_hover_just_init sc_button_hover_style_link3');
        setTimeout(function() {
            jQuery('.sc_button_hover_just_init').removeClass('sc_button_hover_just_init');
        }, 500);
        jQuery('.mejs-controls button,\ .mfp-close,\ .sc_button_bg_image,\ .hover_shop_buttons a,\ button.pswp__button,\ .wp-block-navigation__submenu-icon,\ .woocommerce-orders-table__cell-order-actions .button,\ .sc_layouts_row_type_narrow .sc_button\ ').removeClass('sc_button_hover_' + ALPHA_COLOR_STORAGE['button_hover']);
    }
});
(function() {
    "use strict";
...
(function() {
    "use strict";
    jQuery(document).on('action.ready_alpha_color', function() {
        jQuery('.woocommerce,.woocommerce-page').on('click', '.alpha_color_shop_mode_buttons a', function(e) {
            var mode = jQuery(this).hasClass('woocommerce_thumbs') ? 'thumbs' : 'list';
            alpha_color_set_cookie('alpha_color_shop_mode', mode, 365);
    ...
    jQuery(document).on('action.prepare_stretch_width', function() {
        if (ALPHA_COLOR_STORAGE['stretch_tabs_area'] > 0)
